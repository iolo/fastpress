...
const DEF_CONFIG_FILE = './config.mjs';
const DEF_CONFIG = {
...
export async function initContext(opts) {
  ...
  return { ...config, outDir, staticDir, contentDir, layoutDir, opts };
}
...
async function loadConfig(configFile) {
  ...
  try {
    ...
    // TODO: deep merge?
    return { ...DEF_CONFIG, ...config.default, site: { ...DEF_CONFIG.site, ...config.default.site, } };
  } catch(e) {
...
export async function createPost(context) {
  const { contentDir, opts } = context;

  const title = opts.title ?? 'New Post';
  // YYYY-MM-DD-title-in-kebab-case.md
  const dirname = `${new Date().toISOString().substring(0, 10)}-${title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
  const dir = path.join(path.join(contentDir, 'posts'), dirname);
  await mkdirp(dir);
...
async function copyAssets(staticDir, outDir) {
  ...
  await copyRecursive(staticDir, outDir);
}
...
async function renderEjsPage(content) {
  ...
  return { ...attributes, main: ejs.render(body) };
}

async function renderPugPage(content) {
  const { body, attributes } = frontMatter(content);
  return { ...attributes, main: pug.render(body) };
}
